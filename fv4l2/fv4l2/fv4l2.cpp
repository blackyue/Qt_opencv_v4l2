...
void fv4l2::video_show(){
    FreamBuffer Frame;
    QPixmap pix;

    linux_v4l2_get_one_frame(&Frame);
    pix.loadFromData(Frame.buf, Frame.length);

    ui->showlb->setAlignment(Qt::AlignCenter);  //居中显示
    pix.scaled(ui->showlb->width(),ui->showlb->height(),Qt::KeepAspectRatio);
...
void fv4l2::on_stopbt_clicked()
{
    linux_v4l2_device_release();
}
